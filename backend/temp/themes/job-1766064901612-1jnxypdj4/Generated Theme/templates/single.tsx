  ...
  return (
    <div className="single-template min-h-screen" style={{ backgroundColor: 'rgba(0, 0, 0, 0)', color: 'rgb(68, 68, 68)' }}>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {/* Index/Archive View - Only shown if no single post is active */}
        {!post && (
          <div className="posts-grid grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {posts.filter(p => p.status === 'PUBLISHED').map((post) => (
              <article key={post.id} className="post-card border p-4 rounded-lg hover:shadow-lg transition-shadow">
                {post.featuredImage && (
                  <img 
                    src={post.featuredImage} 
                    alt={post.title} 
